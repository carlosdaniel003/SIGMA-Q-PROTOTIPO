...
def carregar_dados(caminho_arquivo=None):
    ...
    Se nenhum caminho for informado, tenta localizar automaticamente 'base_de_dados.xlsx'
    dentro da pasta do projeto.
    """
    # Caminho padrão (caso o usuário não informe)
    if caminho_arquivo is None:
        caminho_arquivo = os.path.join(os.getcwd(), "data", "base_de_dados.xlsx")

        # Se não existir dentro de /data, tenta na raiz do projeto
        if not os.path.exists(caminho_arquivo):
            caminho_arquivo = os.path.join(os.getcwd(), "base_de_dados.xlsx")
